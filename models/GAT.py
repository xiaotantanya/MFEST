...
class GAT(nn.Module):
    def __init__(self, d_feat=6, hidden_size=64, num_layers=2, dropout=0.0, base_model="GRU", info=''):
        super().__init__()
    ...
    def forward(self, x):
        ...
        hidden = self.fc_out(hidden).squeeze()
        out = self.fc_softmax(hidden)
        return out
...
if __name__ == '__main__':
    ...
    model = GAT(d_feat=4, hidden_size=32, num_layers=2, dropout=0.1).cuda()
    price = torch.randn((101, 30, 4)).cuda()
    out = model(price)
    print(out.shape)